...
For more information on placing spot trades, reference the [Python SDK documentation](/docs/tristero/python-sdk/spot-swaps) or the [API](/docs/tristero/api/getQuote).
...
Tristero's margin feature was built to avoid [ADL](https://thogiti.github.io/2025/12/11/Autodeleveraging-Hyperliquid-653M-debate.html) - an unhealthy market condition in which the positions on one side of a market are forcibly closed on one side of a market in order to maintain solvency, without any gains on the other side. Tristero instead takes a lending based approach - any position onto which a counterparty agrees to allow leverage on will be allowed, avoiding the liquidation risks that are introduced by creating a global orderbook for the asset.

For more information on placing margin trades, reference the [Python SDK documentation](/docs/tristero/python-sdk/margin-trading) or the [API](/docs/tristero/api/getQuote).

### Cross-VM

[Feather](/docs/feather) is a permissionless cross-chain relay integrated into Tristero for swapping into and out of non-EVM assets. No smart contracts, no LPs, no wallet connections — deposit to an address, receive output from Feather's reserves.
...
You can get started with making cross-VM trades by referencing the [Feather Quickstart](/docs/feather/quickstart).
